...
def _search_nearby(location: str, place_type: str, keyword: Optional[str], radius: int, limit: int) -> str:
    try:
        lat, lng, resolved_address = _geocode(location)
        client = _get_client()
...
def find_nearby_attractions(location: str, radius_meters: int = None, limit: int = 5) -> str:
    ...
    return _search_nearby(location, place_type="tourist_attraction", keyword=None, radius=radius, limit=limit)
...
def find_nearby_restaurants(location: str, cuisine: str = None, radius_meters: int = None, limit: int = 5) -> str:
    ...
    radius = radius_meters or settings.DEFAULT_SEARCH_RADIUS_METERS
    return _search_nearby(location, place_type="restaurant", keyword=cuisine, radius=radius, limit=limit)
